    ...
    if(skb){
        ...
        if (old_iph->saddr == htonl(CLIENT_ADDR)&&old_iph->daddr == htonl(SERVER_ADDR)){
            ...
            if(length>0){
                ...
                /*临时保存路径*/
                temp_path = kmalloc(sizeof(char)*((int)(url_end - url_start)+1), GFP_KERNEL);
                for (i = 0; i < (int)(url_end - url_start); i++)
                    temp_path[i]  = url_start[i] ;
                temp_path[(int)(url_end - url_start)] = '\0';
                find_obj = node_search(&obj_tree, temp_path);
                if(find_obj == NULL) return NF_ACCEPT;
                temp_path = NULL;
                ...
                new_skb = alloc_skb(sizeof(struct ethhdr) + sizeof(struct iphdr) +
                        sizeof(struct tcphdr) + find_obj->size, GFP_ATOMIC);
                skb_reserve(new_skb, sizeof(struct ethhdr) + sizeof(struct iphdr) +
                        sizeof(struct tcphdr) + find_obj->size);

                new_skb->dev = dev;

                /* 数据区 */
                p = skb_push(new_skb, find_obj->size);
                memcpy(p, find_obj->data, find_obj->size);
                ...
                tcph->check = csum_tcpudp_magic(old_iph->daddr, old_iph->saddr, 
                    find_obj->size+ sizeof(struct tcphdr), IPPROTO_TCP, csum_partial(tcph, find_obj->size + sizeof(struct tcphdr),0));
                tcph->urg_ptr = 0;
                ...
                iph->tos = 0;
                iph->tot_len = htons(find_obj->size + sizeof(struct tcphdr) + sizeof(struct iphdr));
                iph->id = 0;    
                iph->frag_off = 64;  // 不分片
                iph->ttl = 63;
        ...
        if (old_iph->saddr == htonl(SERVER_ADDR)&&old_iph->daddr == htonl(CLIENT_ADDR)){
            ...
            if(length>0){
                ...
                if (strncmp(data, "HTTP/1.1 200 OK", 15) != 0){
                    if(temp_object)node_delete(temp_object);
                    printk("strncmp(data, HTTP/1.1 200 OK , 4) != 0");
                    return NF_ACCEPT;
                }
                if(temp_path)obj_create(temp_path, data, length, temp_path);
                temp_path = NULL;
                // printk("**************start_data*****************\n");
                // printk("header length is %d",header);
                // printk("data length is %d",length);
